...
def upgrade() -> None:
    ...
    op.create_rls_policy(
        'metadata_select', METADATA_TABLE, command='SELECT', using='true',
    )

    # blog posts: anyone can read
    op.create_rls_policy(
        'blog_select', BLOG_TABLE, command='SELECT', using='true',
    )
    # blog posts: authenticated users can insert their own
    op.create_rls_policy(
        'blog_insert', BLOG_TABLE,
        command='INSERT', to='authenticated',
        with_check='auth.uid() = user_id',
    )
    # blog posts: owner can update
    op.create_rls_policy(
        'blog_update', BLOG_TABLE,
    ...
    op.create_rls_policy(
        'blog_delete', BLOG_TABLE,
        command='DELETE', to='authenticated',
        using='auth.uid() = user_id',
    )
