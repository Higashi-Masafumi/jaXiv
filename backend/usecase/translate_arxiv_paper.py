...
class TranslateArxivPaper:
	...
	async def translate(
		...
		Example:
		    ...
		    >>> )
		    >>> translate_arxiv_paper.translate(
		    ...     arxiv_paper_id=ArxivPaperId(root='1234.5678'),
		...
		tasks = [
			asyncio.create_task(translate_latex_file(latex_file)) for latex_file in latex_files
		]
		...
		try:
			compiled_pdf_file_path = self._latex_compiler.compile(compile_setting=compile_setting)
			# 進捗をストリーミング
