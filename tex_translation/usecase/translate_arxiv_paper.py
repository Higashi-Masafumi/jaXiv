...
class TranslateArxivPaperUseCase:
    """Fetch, translate and compile an arXiv paper. Returns the compiled PDF bytes."""
    ...
    async def execute(
        ...
        # TemporaryDirectory cleans up the workdir on both success and failure,
        # so the controller doesn't need to manage cleanup.
        with tempfile.TemporaryDirectory(prefix="tex-translation-") as output_dir:
            ...
            pdf_path = self._latex_compiler.compile(compile_setting=compile_setting)
